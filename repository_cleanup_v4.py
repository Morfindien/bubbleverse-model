...
SAFE_DELETE = [
    ...
    "BUBBLEVERSE_REPOSITORY_CLEANUP_AUDIT_Q040.json",
]
...
# These files are the cleanup transaction itself. They necessarily contain
# the names of files scheduled for deletion, so they must not be treated as
# external/live references during the pre-delete reference scan.
TRANSACTION_INFRASTRUCTURE = {
    "repository_cleanup.py",
    ".github/workflows/repository-cleanup-once.yml",
    ".github/workflows/repository-cleanup-once-v2.yml",
    ".github/workflows/repository-cleanup-once-v3.yml",
    "BUBBLEVERSE_REPOSITORY_CLEANUP_AUDIT_Q040.json",
}
...
def reference_scan(delete_group: set[str]):
    ...
    for target in sorted(delete_group):
        ...
        for candidate in ROOT.rglob("*"):
            ...
            if rel in delete_group or rel in TRANSACTION_INFRASTRUCTURE:
                continue
